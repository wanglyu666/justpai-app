...
新做页面**除非产品另有安排**，顶部、底部都不要加灰色遮挡（渐隐 mask、铺在滚动层上的灰条、`page-safe-top` 打在滚动容器上等）。

滚动时内容会被这层灰挡住，看起来像被截断。安全区留白写在**会跟着滚的内容里**（例如返回栏自身的 `padding-top`），不要写在外层滚动容器上。底部按钮若要停在页面底，用文档流/flex 占位，不要用灰色渐隐层盖住内容。

已有反例（不要再抄）：新增咨询的 `form-scroll-mask-top` / `form-scroll-mask-bottom`，以及滚动容器上的顶底灰遮罩。新增反馈已按本条去掉顶部遮挡。

## 左缘右滑返回（默认）

凡是带返回按钮的页面，除了点返回，还要从屏幕**左缘向右滑**返回，对齐 App 手势习惯。H5 / 安卓 / iOS 都要能滑。
...
- [ ] 顶部、底部没有灰色遮挡（除非该页另有安排）
- [ ] 带返回按钮的页面，左缘右滑能返回（安卓/iOS 真机）
